...
class Bot(Client):
    ...
    async def start(self):
        ...
        if FORCE_SUB_GROUP6:
            try:
                link = (await self.get_chat(FORCE_SUB_GROUP6)).invite_link
                if not link:
                    await self.export_chat_invite_link(FORCE_SUB_GROUP6)
                    link = (await self.get_chat(FORCE_SUB_GROUP6)).invite_link
                self.invitelink3 = link
            except Exception as a:
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning(
                    "Bot tidak dapat Mengambil link invite dari FORCE_SUB_GROUP6!"
                )
                self.LOGGER(__name__).warning(
                    f"Pastikan @{self.username} adalah admin di Channel Tersebut, Chat ID F-Subs Channel Saat Ini: {FORCE_SUB_GROUP2}"
                )
